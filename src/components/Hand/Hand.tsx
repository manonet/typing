...
import { HandSide, Fingers, PossibleKeyStates } from '@types';
...
export default function Hand({ className, fingers, handSide }: Props) {
  function getFingerClassName(fingerName: Fingers) {
    const finger = fingers && fingers[fingerName];

    if (!finger) {
      return classNames('finger', [fingerName]);
    }
    return classNames('finger', [fingerName], {
      ['finger--missed']: finger.succeedState === 'missed',
      ['finger--correct']: finger.succeedState === 'correct',
      ['finger--error']: finger.succeedState === 'error',
      ['finger--toPressFirst']: finger.marker === 'toPressFirst',
      ['finger--toPressSecond']: finger.marker === 'toPressSecond',
    });
  }
  ...
  return (
    ...
    >
      <g>
        ...
        <path
          className={getFingerClassName('thumb')}
